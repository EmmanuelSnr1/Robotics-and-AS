...
class MyRobot(RASRobot):
    def __init__(self):
        ...
        self.tld = TrafficLightDetector(debug=True)  # Set debug to True if you want visualization and logs
    ...
    def run(self):
        ...
        while self.tick():
            ...
            traffic_light_state = self.tld.detect_traffic_light(image)
            
            print(traffic_light_state)
             # Use lane controller to determine steering angle and speed
            steering_angle, speed = lc.predict_angle_and_speed(image, traffic_light_state)
            
            # Set steering angle and speed of the car
